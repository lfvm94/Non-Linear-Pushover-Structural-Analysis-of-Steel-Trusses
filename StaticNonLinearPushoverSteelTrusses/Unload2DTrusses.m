function [residStr,residDef,defBarHistUnl,StrBarHistUnl]=Unload2DTrusses...
...
%  composing a plane truss through an unloading process from the results 
...
%
%------------------------------------------------------------------------
% LAST MODIFIED: L.F.Veduzco    2022-06-01
...
for i=1:nbars
    ...
    [esBarsNormal(:,i)]=bar2s([ex(ni(i)),ex(nf(i))],[ey(ni(i)),ey(nf(i))],...
        [E(i),A(i)],ue');
...
unlDefBar=(Lnew-L)./L; % Axial deformation (positive when the 
                    % bar is in tension)
...
if isempty(barsplotPD)==0
    ...
    residDef=defBarHist(:,plasticNsteps+1)+esBarsNormal(1,:)'./A./E;
